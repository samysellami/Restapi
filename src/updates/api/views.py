...
class UpateModelDetailApiView(CSRFExemptMixin, HttpResponseMixin, View):
	'''
	...
	def put(self, request, id, *args, **kwargs):
		...
		if form.errors:
			data = json.dumps(forms.errors)
			return self.render_to_response(data, status = 400)

		json_data = json.dumps({"message":"someting"})
...
class UpateModelListApiView(CSRFExemptMixin, HttpResponseMixin, View):
	'''
		List view, 
		Create view
		Update
	...
	def put(self, request, *args, **kwargs):
		...
		if not passed_id:
			error_data = json.dumps({"id": "This is a required field to update the item."})
			return self.render_to_response(error_data, status = 400)
